...
parser.add_argument('--model', default='model_part_seg', help='Model name [default: model]')
parser.add_argument('--log', default='log', help='Log dir [default: log]')
parser.add_argument('--num_point', type=int, default=2048, help='Point Number [default: 2048]')
parser.add_argument('--max_epoch', type=int, default=201, help='Epoch to run [default: 201]')
parser.add_argument('--batch_size', type=int, default=8, help='Batch Size during training [default: 32]')  # 16
...
def train():
    with tf.Graph().as_default():
        ...
        for epoch in range(MAX_EPOCH):
            ...
            best_mean_iou_flag, best_all_mean_iou_flag, best_all_mean_iou = eval_one_epoch(sess, ops, test_writer)
...
def eval_one_epoch(sess, ops, test_writer):
    ...
    for batch_idx in range(num_batches):
        ...
        if CLS_FLAG > 0:
            ...
            summary, step, loss_val, pred_val, cls_pred_val = sess.run([ops['merged'], ops['step'],
                                                                        ops['loss'], ops['pred'], ops['cls_pred']],
                                                                       feed_dict=feed_dict)
        else:
            feed_dict = {ops['pointclouds_pl']: batch_data,
                         ops['labels_pl']: batch_label,
    ...
    return (best_mean_iou_flag, best_all_mean_iou_flag, BEST_ALL_MEAN_IOU)
